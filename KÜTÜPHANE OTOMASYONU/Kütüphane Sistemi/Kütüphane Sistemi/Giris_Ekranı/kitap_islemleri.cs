...
namespace Giris_Ekranı
{
    public partial class kitap_islemleri : Form
    {
        ...
        private DataTable filteredTable = new DataTable(); // Filtrelenmiş veriyi saklamak için değişken
        ...
        {
            // Kitap adını TextBox'tan al
            string kitapAd = textBox28.Text.Trim();
        ...
        private void CallStoredProcedure(string kitapAd)
        {
            try
            {
                using (SqlConnection conn = SqlHelper.GetConnection()) // Veritabanı bağlantısı
                {
                    ...
                    // Stored procedure parametresini ekle
                    cmd.Parameters.AddWithValue("@kitapAd", kitapAd);
                    ...
                    // Sonuç boş ise kullanıcıya bilgi ver
                    if (filteredTable.Rows.Count == 0)
                    {
                        MessageBox.Show("Eşleşen kitap bulunamadı.", "Bilgi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            catch (Exception ex)
    ...
    byte kutuphaneID, string yazarAd, string yazarSoyad, DateTime basimYil, int sayfaSayi, string yayinEvi, bool raftaDurum)
        ...
        {
            if (filteredTable.Rows.Count > 0) // Önce arama yapılmış ve sonuçlar alınmış mı kontrol edin
            {
                rapor3 rapor = new rapor3();
                rapor.SetFilteredDataTable(filteredTable); // Filtrelenmiş tabloyu rapor formuna aktar
                rapor.ShowDialog();
            }
            else
            {
                MessageBox.Show("Rapor göstermek için önce bir kitap araması yapınız.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
    }
}
