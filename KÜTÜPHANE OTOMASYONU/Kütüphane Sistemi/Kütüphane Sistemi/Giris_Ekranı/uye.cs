...
namespace Giris_Ekranı
{
    ...
    {
        private DataTable filteredTable = new DataTable(); // Filtrelenmiş veri burada saklanacak
        ...
        private void uyeekle_button_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (uyeTcNo.Length != 11 || !uyeTcNo.All(char.IsDigit))
                {
                    MessageBox.Show("TC Kimlik Numarası 11 haneli olmalıdır.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                // E-posta doğrulaması: "@gmail.com" ifadesi içeriyor mu kontrol et
                if (!uyeEposta.Contains("@gmail.com"))
                {
                    MessageBox.Show("E-posta adresi '@gmail.com' olmalıdır.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                using (SqlConnection con = SqlHelper.GetConnection())
                {
                    ...
                    string checkQuery = "SELECT uyeTcNo, uyeEposta, uyeCepTelefon FROM Uye WHERE uyeTcNo = @uyeTcNo OR uyeEposta = @uyeEposta OR uyeCepTelefon = @uyeCepTelefon";
                    ...
                    using (SqlDataReader reader = checkCmd.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                if (reader["uyeTcNo"] != DBNull.Value && reader["uyeTcNo"].ToString() == uyeTcNo)
                                {
                                    MessageBox.Show("Bu TC Kimlik Numarası zaten kayıtlı. Lütfen farklı bir TC Kimlik Numarası giriniz.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    return;
                                }

                                if (reader["uyeCepTelefon"] != DBNull.Value && reader["uyeCepTelefon"].ToString() == uyeCepTelefon)
                                {
                                    MessageBox.Show("Bu cep telefonu numarası zaten kayıtlı. Lütfen farklı bir cep telefonu numarası giriniz.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    return;
                                }

                                if (reader["uyeEposta"] != DBNull.Value && reader["uyeEposta"].ToString() == uyeEposta)
                                {
                                    MessageBox.Show("Bu e-posta adresi zaten kayıtlı. Lütfen farklı bir e-posta adresi giriniz.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    return;
                                }
                            }
                        }
        ...
        private void uyeara_button_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                string uyeKullaniciAd = textBox8.Text.Trim();
                string uyeKullaniciSoyad = textBox9.Text.Trim();
                ...
                using (SqlConnection con = SqlHelper.GetConnection())
                {
                    ...
                    filteredTable.Clear(); // Önceki veriyi temizle
                    da.Fill(filteredTable);
            ...
            if (dataGridView2.SelectedRows.Count > 0)
            {
                ...
                try
                {
                    ...
                    using (SqlConnection conn = SqlHelper.GetConnection())
                    {
                        ...
                        if (uyeCepTelefon != existingUyeCepTelefon)
                        {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (filteredTable.Rows.Count > 0)
            {
                ...
                rapor.SetFilteredDataTable(filteredTable); // Filtrelenmiş tabloyu rapor formuna gönder
                rapor.ShowDialog();
            }
            else
            {
                MessageBox.Show("Rapor göstermek için önce bir arama yapmalısınız!", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
    }
}
